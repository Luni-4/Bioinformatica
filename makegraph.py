...
def cmp_ills(ont, learner_fam, prefix = 'M_'):
    fig, ax = plt.subplots()
    files = []
    for entry in os.scandir(inputfolder + ont):
        if entry.name.endswith('.json') and entry.name.startswith(prefix) and entry.is_file():
    ...
    if prefix == 'LS':
        fig.savefig(outputfolder + ont + '-' + learner_fam + '-ills-LS.eps')
    else:
...
def level1(ont, learner_fam, prefix = 'M_'):
    ...
    #fig.show()
    if prefix == 'LS':
        fig.savefig(outputfolder + ont + '-' + learner_fam + '-level1-LS.eps')
    else:
...
if __name__ == '__main__':
    for ont in ['CC']:
        for learner_fam in ['AdaBoost', 'SVM', 'Pegasos']:
            ...
            pass
        level1(ont, 'Pegasos', prefix = 'LS')
        cmp_ills(ont, 'Pegasos', prefix = 'LS')
        level3(ont)
        level2(ont)
