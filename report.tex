...
\paragraph*{}
La \textit{F-score} è data dalla media armonica della precision e della recall. È definita nell'intervallo $[0,1]$. Più la f-score è alta, più un classificatore produrrà risultati migliori.
...
\paragraph*{}
L'\textit{Area Under the Precision-Recall Curve (AUPRC)} rappresenta l'area sotto la curva del grafico precision-recall. Assume valori definiti nell'intervallo $[0,1]$. Più il valore è alto, più il classificatore produrrà buoni risultati.

\paragraph*{}
L'\textit{Area Under the Receiver Operating Characteristic (AUROC)} rappresenta la probabilità che un classificatore associ un valore più alto ad un esempio positivo scelto casualmente rispetto ad uno negativo, sempre scelto in maniera casuale. Assume valori definiti nell'intervallo $[0,1]$. Più il valore è alto, più il classificatore produrrà buoni risultati.

\paragraph*{}
Come spiegato precedentemente, ciascuna metrica viene calcolata per ogni fold di una classe e i valori ottenuti vengono salvati in un file json. I risultati prodotti sono letti in maniera sequenziale e, per ogni classe, viene calcolata la media tra le metriche dei fold. Ai valori ottenuti sarà nuovamente applicato l'operatore di media per individuare il valor medio che descrive il classificatore su tutte le classi.
