...
Le metriche sono state scelte tenendo in considerazione diversi aspetti del dataset.
A causa della presenza di classi sbilanciate, con un numero elevato di esempi negativi rispetto ai positivi, l'accuratezza risulta una metrica non significativa per il modello che si sta valutando.

\paragraph*{}
L'accuratezza rappresenta la frazione delle predizioni corrette ed è definita come:

\begin{equation}
\texttt{accuracy}(y, \hat{y}) = \frac{1}{n_\text{samples}} \sum_{i=0}^{n_\text{samples}-1} 1(\hat{y}_i = y_i)
\end{equation}

Nella formula soprastante $1(x)$ è la funzione indicatrice. Come si può notare, l'accuratezza fornisce un buon valore nel caso di sbilanciamento, in quanto non discrimina tra valori positivi e negativi nell'effettuare la misurazione.

\paragraph*{}
Le metriche considerate nel progetto sono la precision, la recall, la f-score, l'AUPRC e l'AUROC.
...
\paragraph*{}
Come spiegato precedentemente, le metriche sono calcolate per ogni fold di una classe e i risultati sono salvati in un file json. Per poter valutare le performance di un classificatore di una classe, viene fatta la media delle metriche di ciascun fold associato alla classe stessa.

\section{Risultati}
Per identificare i migliori classificatori e poterli confrontare tra loro, è stata creata una struttura a tre livelli. Nei primi due livelli i classificatori appartenenti a modelli diversi non interagiscono tra loro. In ogni livello, le ontologie sono sempre analizzate in maniera indipendente.  
...
\paragraph*{}
Il \textit{Livello 2} confronta la configurazione del classificatore che ha ottenuto i risultati migliori al livello precedente su diverse classi. Lo scopo di questa analisi consiste nel capire come un classificatore varia le sue predizioni a seconda della classe scelta. Le classi considerate sono tre e sono disposte in ordine crescente rispetto al numero di esempi positivi.
...
\begin{figure}[ht]%
    ...
    \caption{Confronto tra i classificatori che hanno le migliori performance su diverse classi dell'ontologia CC. Le classi sono in ordine crescente rispetto al numero di positivi.}%
    \label{fig:liv2} 
\end{figure}
...
\begin{figure}[hb]%
    ...
    \caption{Confronto tra i classificatori che hanno le migliori performance su diverse classi dell'ontologia MF. Le classi sono in ordine crescente rispetto al numero di positivi.}%
    \label{figure:liv21}%
\end{figure}
